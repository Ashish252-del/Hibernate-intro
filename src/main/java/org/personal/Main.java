...
//     // session means you are basically asking for a new connection for database
     ...
     users st = new users();
     st.setId(2);
     st.setFirstname("Ankit");
     st.setLastname("Patel");
     st.setEmail("ashishpatel3945@gmail.com");
     st.setMobile("8869937302");
     session.beginTransaction(); // here we are telling that we nee to proceed a query
     session.save(st);
